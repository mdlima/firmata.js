...
var DHTSENSOR_ATTACH_DHT11 = 0x01;
var DHTSENSOR_ATTACH_DHT22 = 0x02;
var DHTSENSOR_DATA = 0x74;
var DHTSENSOR_DETACH = 0x03;
var DHTSENSOR_RESPONSE = 0x00;
var DIGITAL_MESSAGE = 0x90;
var END_SYSEX = 0xF7;
var ANALOG_READ = 0x66;
var ANALOG_READ_QUERY = 0x00;
var ANALOG_READ_RESPONSE = 0x01;
...
var MOISTURESENSOR_CMDID = 0x0200;
var MOISTURESENSOR_RESPONSE = 0x00;
var MOISTURESENSOR_ATTACH = 0x01;
var MOISTURESENSOR_DETACH = 0x02;
var MOISTURESENSOR_QUERY = 0x03;
...
/**
 * SYSEX_EXTENDED_ID_RESPONSE contains functions to be called when we receive a SYSEX ESTENDED_ID message from the arduino.
 * used as a switch object as seen here http://james.padolsey.com/javascript/how-to-avoid-switch-case-syndrome/
 * @private
 */

var SYSEX_EXTENDED_ID_RESPONSE = {};

/***
 * REPORT:
 * 1. START_SYSEX                       0xF0
 * 2. SYSEX_EXTENDED_ID                 0x00
 * 3. MOISTURESENSOR_CMDID_FIRST_PART   0x02
 * 4. MOISTURESENSOR_CMDID_SECOND_PART  0x00
 * 5. MOISTURESENSOR_RESPONSE           0x00
 * 6. analog pin                       (0-127)
 * 7. value                             lsb
 * 8. value                             msb
 * 9. END_SYSEX
***/
SYSEX_EXTENDED_ID_RESPONSE[MOISTURESENSOR_CMDID] = function(board) {
  // console.log("MOISTURESENSOR_CMDID:", board.currentBuffer.map(b => b.toString(16)).join("|"));
  var command = board.currentBuffer[4] & 0x7F;
  if (command !== MOISTURESENSOR_RESPONSE) return;
  var pin = board.currentBuffer[5] & 0x7F;
  var value = board.currentBuffer[6] | (board.currentBuffer[7] << 7);
  board.emit("moisture-sensor-response-" + pin, value);
  board.emit("moisture-sensor-response", {
    pin: pin,
    value: value,
  });
}

// Board.SYSEX_EXTENDED_ID_RESPONSE = SYSEX_EXTENDED_ID_RESPONSE;
...
/**
 * Handles a SYSEX_EXTENDED_ID response and search for the SYSEX_EXTENDED_ID_RESPONSE handler to be executed
 * @private
 * @param {Board} board the current arduino board we are working with.
 */

SYSEX_RESPONSE[SYSEX_EXTENDED_ID] = function(board) {
  var command = ((board.currentBuffer[2] & 0x7F) << 8) | (board.currentBuffer[3] & 0x7F);

  var handler = SYSEX_EXTENDED_ID_RESPONSE[command];
...
/**
 * Handles an ANALOG_READ RESPONSE and emits "analog-read" and "analog-read-"+n events where n is the pin number.
 * @private
 * @param {Board} board the current arduino board we are working with.
 */

SYSEX_RESPONSE[ANALOG_READ] = function(board) {
  var subCommand = board.currentBuffer[2];

  if (subCommand === ANALOG_READ_RESPONSE && board.currentBuffer.length >= 5) {
    var pin = board.currentBuffer[3] & 0x7F;
    var value = 0;

    var numberBuffer = board.currentBuffer.slice(4, -1);
...
/**
 * Handles a DHTSENSOR_DATA response and emits "dht-read" and "dht-read-"+n events where n is the pin number.
 * @private
 * @param {Board} board the current arduino board we are working with.
 */
SYSEX_RESPONSE[DHTSENSOR_DATA] = function(board) {
  var command = board.currentBuffer[2] & 0x7F;
  if (command !== DHTSENSOR_RESPONSE) return;
  var pin = board.currentBuffer[3] & 0x7F;
  var tempValue = decode14BitSignedInteger(board.currentBuffer.slice(4, 6)) / 10.0;
  var humidityValue = decode14BitSignedInteger(board.currentBuffer.slice(6, 8)) / 10.0;

  // board.emit("dht-read-" + pin, tempValue, humidityValue);
  // console.log("Data coming from DHT sensor. temperature ", tempValue, "humidity", humidityValue);
  board.emit("dht-read", {
    pin: pin,
    tempValue: tempValue,
...
Board.prototype.analogRead = function(pin, callback) {
  if (pin <= 15) {
    ...
    this.extendedAnalogRead(pin, { enableReporting: true }, callback);
  }
};

/**
 * Asks the arduino to read analog data using ANALOG_READ. Reporting is optional.
 * @param {number} pin The pin to read analog data
 * @param {object} options Options:
 *   enableReporting {boolean} Enable/disable reporting for the pin
 * @param {function} callback A function to call when we have the analag data.
 */

Board.prototype.extendedAnalogRead = function(pin, options, callback) {
  var data = [START_SYSEX, ANALOG_READ, ANALOG_READ_QUERY, pin & 0x7f];

  if (typeof options === "function" || typeof options === "undefined") {
    callback = options;
    options = {};
  } else if (options && typeof options.enableReporting !== "undefined") {
    data.push(options.enableReporting ? 0x01 : 0x00)
  }
  data.push(END_SYSEX);
  ...
  console.log("extendedAnalogRead:", data.map(b => b.toString(16)).join("|"));
};
...
/**
 * DHT sensor functions to support ConfigurableFirmata's connection to DHT sensors
 * https://github.com/firmata/ConfigurableFirmata
 */

/**
 * Asks the arduino to configure a DHT sensor with the given config
 * @param {number} pin Pin connected to the data line on the sensor. Must be an Interrupt pin.
 * @param {number} type One of this.DHT_SENSOR_TYPES.*
 * @param {boolean} blockingReads Enables blocking during sensor reads. Use with caution.
 * @param {number} samplingInterval How often (in ms) should Firmata poll the sensor and send data.
 */

Board.prototype.dhtSensorAttach = function(pin, type, blockingReads = null, samplingInterval = null) {
  var data = [
    START_SYSEX,
    DHTSENSOR_DATA,
    (type === this.DHT_SENSOR_TYPES.DHT11) ? DHTSENSOR_ATTACH_DHT11 : DHTSENSOR_ATTACH_DHT22,
    pin,
  ];
  if (blockingReads !== null) data.push(blockingReads ? 0x01 : 0x00);
  if (samplingInterval !== null) Array.prototype.push.apply(data, encode32BitSignedInteger(samplingInterval));
  data.push(END_SYSEX);
  writeToTransport(this, data);
};

/**
 * Asks the arduino to detach an existing DHT sensor
 */

Board.prototype.dhtSensorDetach = function() {
  var data = [
    START_SYSEX,
    DHTSENSOR_DATA,
    DHTSENSOR_DETACH, // STEPPER_STEP from firmware
    END_SYSEX
  ];
  writeToTransport(this, data);
};
...
/**
 * MoistureSensor functions to support connecting to moisture sensors
 */

/**
 * Asks the arduino to configure a moisture sensor to enable easy querying of the sensor
 * @param {number} analogPin The analog pin that the sensor is connected to
 * @param {number} powerPinA (optional) The pin to power the moisture sensor, so that it's powered only when reading
 * @param {number} powerPinB (optional) The other pin to power the moisture sensor, so that the sensor can be powered with AC and prevent eletrolisis build-up
 */
 Board.prototype.moistureSensorAttach = function(analogPin, powerPinA = null, powerPinB = null) {
  /***
   *  Handle MOISTURESENSOR_FEATUREID (0x0101) sysex extended commands
   * 
   * ATTACH:
   * 1. START_SYSEX                       0xF0
   * 2. SYSEX_EXTENDED_ID                 0x00
   * 3. MOISTURESENSOR_CMDID_FIRST_PART   0x02
   * 4. MOISTURESENSOR_CMDID_SECOND_PART  0x00
   * 5. MOISTURESENSOR_ATTACH             0x01
   * 6. analog pin                       (0-127)
   * 7. power pin A                      (0-127) [optional - not used if moisture sensor is connected to Vcc]
   * 8. power pin B                      (0-127) [optional - not used if moisture sensor is connected to Vcc or only one power pin]
   * 9. END_SYSEX
   */
  var data = [
    START_SYSEX,
    SYSEX_EXTENDED_ID,
    (MOISTURESENSOR_CMDID >> 8) & 0x7F,
    MOISTURESENSOR_CMDID & 0x7F,
    MOISTURESENSOR_ATTACH,
    analogPin,
  ];
  if (powerPinA !== null) data.push(powerPinA);
  if (powerPinB !== null) data.push(powerPinB);
  data.push(END_SYSEX);
  // console.log("moistureSensorAttach:", data.map(b => b.toString(16)).join("|"));
  writeToTransport(this, data);
};
 ...
 * 3. MOISTURESENSOR_CMDID_FIRST_PART   0x02
 * 4. MOISTURESENSOR_CMDID_SECOND_PART  0x00
 * 5. MOISTURESENSOR_QUERY              0x03
 * 5. sensor analog pin                (0-127) [1-126 = query a single sensor, 127 = query all attached sensors, multiple messages will be sent]
 * 6. number of readings               (1-31) [optional - number of readings to be done and have the average returned, default = 1]
 * 7. END_SYSEX
 */
Board.prototype.moistureSensorQuery = function(analogPin, numReadings = null, callback) {
  var data = [
    START_SYSEX,
    SYSEX_EXTENDED_ID,
    (MOISTURESENSOR_CMDID >> 8) & 0x7F,
    MOISTURESENSOR_CMDID & 0x7F,
    MOISTURESENSOR_QUERY,
    analogPin,
  ];
  if (numReadings !== null) data.push(numReadings);
  data.push(END_SYSEX);
  this.once("moisture-sensor-response-" + analogPin, callback);
  // console.log("moistureSensorQuery:", data.map(b => b.toString(16)).join("|"));
  writeToTransport(this, data);
};
 ...
 * 3. MOISTURESENSOR_CMDID_FIRST_PART   0x02
 * 4. MOISTURESENSOR_CMDID_SECOND_PART  0x00
 * 5. MOISTURESENSOR_DETACH             0x02
 * 6. analog pin                       (0-127)
 * 7. END_SYSEX
 */
Board.prototype.moistureSensorDetach = function(analogPin, numReadings = null) {
  var data = [
    START_SYSEX,
    SYSEX_EXTENDED_ID,
    (MOISTURESENSOR_CMDID >> 8) & 0x7F,
    MOISTURESENSOR_CMDID & 0x7F,
    MOISTURESENSOR_DETACH,
    analogPin,
    END_SYSEX
  ];
  // console.log("moistureSensorDetach:", data.map(b => b.toString(16)).join("|"));
  writeToTransport(this, data);
};
...
function decode14BitSignedInteger(bytes) {
  // Decode considering 2 bytes with 7 bits each composing a 14 bit integer
  var result = (bytes[0] & 0x7f) |
    ((bytes[1] & 0x7f) << 7);
