...
MIDI_RESPONSE[USER_COMMAND_DHT] = function(board) {
  var pin = board.currentBuffer[0] & 0x0F;
  var humidityValue = board.currentBuffer[1] | (board.currentBuffer[2] << 7);
  var tempValue = board.currentBuffer[3] | (board.currentBuffer[4] << 7);
  var heatIndexValue = board.currentBuffer[5] | (board.currentBuffer[6] << 7);

  board.emit("dht-read-" + pin, humidityValue, tempValue, heatIndexValue);
  board.emit("dht-read", {
    pin: pin,
    humidityValue: humidityValue / 100,
    tempValue: tempValue / 100,
    heatIndexValue: heatIndexValue / 100,
  });
};
