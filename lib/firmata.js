...
/**
 * Handles a USER_COMMAND_DHT response and emits "dht-read" and "dht-read-"+n events where n is the pin number.
 * @private
 * @param {Board} board the current arduino board we are working with.
 */

MIDI_RESPONSE[USER_COMMAND_DHT] = function(board) {
  var pin = board.currentBuffer[0] & 0x0F;
  var humidity_value = board.currentBuffer[1] | (board.currentBuffer[2] << 7);
  var temp_value = board.currentBuffer[3] | (board.currentBuffer[4] << 7);
  var heat_index_value = board.currentBuffer[5] | (board.currentBuffer[6] << 7);

  board.emit("dht-read-" + pin, humidity_value, temp_value, heat_index_value);
  board.emit("dht-read", {
    pin: pin,
    humidity_value: humidity_value,
    temp_value: temp_value,
    heat_index_value: heat_index_value,
  });
};
