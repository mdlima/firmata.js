...
/**
 * Handles an EXTENDED_ANALOG_READ RESPONSE and emits "analog-read" and "analog-read-"+n events where n is the pin number.
 * @private
 * @param {Board} board the current arduino board we are working with.
 */

SYSEX_RESPONSE[EXTENDED_ANALOG_READ] = function(board) {
  var subCommand = board.currentBuffer[2];

  if (subCommand === EXTENDED_ANALOG_READ_RESPONSE && board.currentBuffer.length >= 5) {
...
/**
 * Asks the arduino to read analog data using EXTENDED_ANALOG_READ. Reporting is optional.
 * @param {number} pin The pin to read analog data
...
Board.prototype.extendedAnalogRead = function(pin, options, callback) {
  var data = [START_SYSEX, EXTENDED_ANALOG_READ, EXTENDED_ANALOG_READ_QUERY, pin & 0x7f];
  ...
  data.push(END_SYSEX);
  console.log("EXTENDED_ANALOG_READ: ", data.map(v => v.toString(16)).join("|"));
