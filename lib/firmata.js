...
var DHTSENSOR_ATTACH_DHT22 = 0x00;
var DHTSENSOR_DATA = 0x66;
var DHTSENSOR_DETACH = 0x00;
...
SYSEX_RESPONSE[DHTSENSOR_DATA] = function(board) {
  // var pin = board.currentBuffer[2] & 0x7F;
  var tempValue = decode14BitSignedInteger(board.currentBuffer.slice(2, 4)) / 10;
  var humidityValue = (board.currentBuffer[4] | (board.currentBuffer[5] << 7)) / 100;
...
Board.prototype.analogRead = function(pin, callback) {
  this.analogReadCustomInterval(pin, 1, false, callback);
};

/**
 * Asks the arduino to read analog data. Turn on reporting for this pin.
 * @param {number} pin The pin to read analog data
 * @param {number} interval Sampling interval for pin in millis or seconds. If 1ms, then global sampling interval will be used.
 * @param {boolean} interval_unit_s false if interval is in millis, true if in seconds. Default is millis.
 * @param {function} callback A function to call when we have the analag data.
 */

Board.prototype.analogReadCustomInterval = function(pin, interval, interval_unit_s, callback) {
  this.reportAnalogPin(pin, interval, interval_unit_s);
...
Board.prototype.setSamplingInterval = function(interval) {
  var safeint = interval < 10 ? 10 : (interval > 16383 ? 16383 : interval);
 ...
 * @param {number} value Sampling interval for pin in millis or seconds. If 1ms, then global sampling interval will be used.
 * @param {boolean} value_unit_s false if interval is in millis, true if in seconds. Default is millis.
  */

Board.prototype.reportAnalogPin = function(pin, value, value_unit_s = false) {
  /* istanbul ignore else */
  if (value >= 0) {
    var safeint = value & 0x1FFF;
    this.pins[this.analogPins[pin]].report = safeint;
    this.pins[this.analogPins[pin]].reportInterval = value_unit_s ? safeint * 1000 : safeint;
    if (safeint === 1 && !value_unit_s) {
      // Use shorter message
      writeToTransport(this, [REPORT_ANALOG | pin, safeint]);
    } else {
      writeToTransport(this, [
        START_SYSEX,
        REPORT_ANALOG_CONFIG,
        pin,
        (safeint & 0x7F),
        (((safeint >> 7) & 0x3F) | (value_unit_s ? 0x40 : 0x00)),
        END_SYSEX
      ]);
    }
  }
 ...
 * @param {number} samplingInterval How often should Firmata poll the sensor and send data.
 * @param {boolean} samplingIntervalUnitSeconds Tells which unit to use for samplingInterval. false = millis, true = seconds.
 */

Board.prototype.dhtSensorAttach = function(pin, type, samplingInterval, samplingIntervalUnitSeconds) {
  var data = [
    START_SYSEX,
    DHTSENSOR_DATA,
    ...
    pin,
    samplingInterval,
    samplingIntervalUnitSeconds ? 1 : 0,
    END_SYSEX
  ];
  writeToTransport(this, data);
};
